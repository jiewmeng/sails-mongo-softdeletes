...
Collection.prototype.find = function find(criteria, cb) {
  criteria.where = criteria.where ? _.assign(criteria.where, {deletedAt: null}) : {deletedAt: null};
  var self = this,
      query;
...
Collection.prototype.insert = function insert(values, cb) {
  ...
  var docs = values.map(function(value) {
    return _.assign(new Document(value, self.schema).values, { deletedAt: null });
  });
...
Collection.prototype.destroy = function destroy(criteria, cb) {
  ...
  var collection = this.connection.db.collection(self.identity);
  collection.update(query.criteria.where, {'$set': {deletedAt: new Date()}}, {multi: true}, function(err, results) {
    if(err) return cb(err);
